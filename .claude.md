# Portland Fresh - Technical Architecture & Best Practices

**Last Updated**: 2025-11-17
**Project**: Portland Fresh E-commerce Platform
...
#### ✅ Supabase (PRIMARY - Products & E-commerce)
...
- ✅ `/blends` (Products listing page)
- ✅ `/blends/[slug]` (Individual product pages)
...
#### ✅ Sanity CMS (SECONDARY - Content Pages)
...
- ✅ `/how-we-make-it` - Process page content
- ✅ `/ingredients` - Ingredients & sourcing page content
- ✅ `/about` - About page content
- ✅ `/journal` - Blog listing
- ✅ `/journal/[slug]` - Individual blog posts
- ✅ `/(website)/layout.tsx` - Navigation and site settings
- ✅ `/admin/stripe` - Stripe settings UI

---

## ❌ REDUNDANCIES & DEPRECATED

### ✅ CLEANUP COMPLETED: Removed Deprecated Schemas

**The following Sanity schemas have been DELETED** (as of 2025-11-17):
- ❌ `blend.ts` - Deleted (products now in Supabase)
- ❌ `ingredient.ts` - Deleted (ingredients now in Supabase)
- ❌ `farm.ts` - Deleted (not used)
- ❌ `sizePrice.ts` - Deleted (pricing now in Supabase)

**Cleaned up**:
- ✅ Removed `featuredBlends` reference from `homePageQuery` in `lib/sanity.queries.ts`
- ✅ Homepage now uses `getFeaturedProducts()` from Supabase
- ✅ All product-related queries removed from Sanity

**Kept**:
- ✅ `blendsPage.ts` - Page metadata (heading, subheading, SEO) - NOT product data
...
**❌ WRONG** - Don't fetch products from Sanity:
```typescript
// DON'T DO THIS
...
**✅ CORRECT** - Always use Supabase for products:
```typescript
import { getAllProducts } from '@/lib/supabase/queries/products';
const products = await getAllProducts();
```
...
/app
  /(website)
    /blends              → Supabase products
    /blends/[slug]       → Supabase products
    /faq                 → Sanity content
    /how-we-make-it      → Sanity content
    /ingredients         → Sanity content
    /journal             → Sanity posts
    /journal/[slug]      → Sanity posts
    page.tsx             → Sanity homepage + Supabase featured products (HYBRID)
  /api
    /checkout            → Stripe + Supabase
    /stripe/webhook      → Stripe + Supabase
  sitemap.ts             → Supabase products
  robots.ts              → Static

/lib
  /supabase
    /queries
      products.ts        → Product queries (PRIMARY)
  sanity.client.ts       → Sanity client
  sanity.queries.ts      → Content queries (SECONDARY)
...
**Integration Points**:
- `lib/actions.ts` - Newsletter & wholesale forms (✅ Connected)
- `lib/email/send.ts` - Order confirmations (✅ Connected to webhook)
- `/api/stripe/webhook/route.ts` - Sends order emails (✅ Connected)
...
2. ✅ Ensure Supabase has all products configured
3. ✅ Verify Stripe webhook is pointing to production URL
4. ✅ Set `RESEND_API_KEY` for email notifications
5. ✅ Verify `EMAIL_FROM` domain in Resend
6. ⚠️ Consider deprecating/removing old Sanity product schemas
7. ⚠️ Run `npm run build` to verify 0 TypeScript errors
...
1. ❌ Create products in Sanity CMS (those schemas have been deleted)
2. ❌ Use `console.log` in production code
3. ❌ Skip TypeScript types (`any` everywhere)
4. ❌ Mix Sanity and Supabase for the same data type

---

## ✅ DO

1. ✅ Create products in Supabase database
2. ✅ Use `getAllProducts()` for product lists
3. ✅ Use Sanity ONLY for content pages (FAQ, About, Blog)
4. ✅ Use `logger` utility for all logging
